...
<div id="<?php echo $id ?>-container" <?php if ( isset( $deps ) ): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $deps['ids'] ?>" data-value="<?php echo $deps['values'] ?>" <?php endif ?>class="select_icon rm_option rm_input rm_text">
    ...
    <div class="option">
        ...
        <div class="input_wrapper custom_icon_wrapper upload" style="clear:both;">
            ...
            <input type="button" value="<?php _e( 'Upload', 'yith_wc_tab_manager' ) ?>" id="<?php echo $id; ?>-custom-button" class="upload_button button" />
            ...
            <div class="upload_img_preview" style="margin-top:10px;">
                ...
                ?>
                <?php _e('Image preview', 'yith_wc_tab_manager') ?> : <img src="<?php echo $file; ?>"  style="max-width :27px;max-height: 25px;"/>
            </div>
        </div>
    ...
    <div class="description">
        ...
        <?php if( $std['select'] == 'custom' ) : ?>
            <?php printf( __( '(Default: %s <img src="%s"/>)', 'yith_wc_tab_manager' ), $options['select']['custom'], $std['custom'] ) ?>
        <?php else: ?>
            <?php printf( __( '(Default: <i %s></i> )', 'yith_wc_tab_manager' ), $current_icon  ) ?>
        <?php endif; ?>
    </div>
